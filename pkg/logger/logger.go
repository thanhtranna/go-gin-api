...
func WithFileRotationP(file string) Option {
	...
	return func(opt *option) {
		opt.file = &lumberjack.Logger{ // concurrent-safed
			...
			MaxSize:    128,  // The maximum size of a single file, the default unit is M
			MaxBackups: 300,  // Keep up to 300 backups
			MaxAge:     30,   //Maximum time, the default unit is day
			LocalTime:  true, // use local time
			Compress:   true, // Whether to compress disabled by default
		}
	}
}
